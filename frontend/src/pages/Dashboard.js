              ...
              <tbody>
                {clients.map(client => (
                  <tr key={client.id}>
                    ...
                    <td>
                      <div style={{ color: 'var(--success)', fontWeight: 700 }}>{formatCurrency(client.total_earnings, fmt, currency, exchangeRate)}</div>
                      {fmt !== 'exact' && <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{exactCurrency(client.total_earnings, currency, exchangeRate)}</div>}
                    </td>
                    <td>
                      <button
